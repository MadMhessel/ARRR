...
- **Контур**: насыщённый чёрный `#111111`, толщина 1.8 px для мебели и 3.3 px для капитальных стен. Контуры не масштабируются (`vector-effect: non-scaling-stroke`).
- **Акцентные детали**: тот же чёрный `#111111`, толщина 2.2 px (≈1.2× к контуру). Используются для распознаваемых штрихов каждого типа: спинки стульев, рабочие кромки, ручки и полки.
- **Вспомогательные линии**: холодный серый `#676D72`, толщина 1.2 px. Это оси, дуги дверей, штриховка стекла, вспомогательные панели.
...
| Элемент | Значение |
| --- | --- |
| Капитальные стены | Контур `#111111`, расчётная толщина 0.25 м (≈3.3 px).
| Перегородки | Толщина 0.16 м (≈2.1 px), тот же цвет; стекло — `#676D72` с штриховкой.
| Контур мебели | 1.8 px, цвет `#111111`.
| Акцентные признаки | 2.2 px, цвет `#111111`.
| Вспомогательные линии | 1.2 px, цвет `#676D72`, пунктир 6/4 для дуг дверей.
