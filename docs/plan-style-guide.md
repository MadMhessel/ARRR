...
### Рабочее поле и центр (0;0)

* Вся графика предмета помещается в прямоугольник `[-w/2; w/2] × [-h/2; h/2]`. Свободный зазор до рамки — не менее 2 px, чтобы при печати не возникал клиппинг.
* Трансформации выполняются в порядке «центрирование → масштаб → базовая трансформация». `applyTransformFromDataset()` использует сохранённый `data-base-transform`, поэтому после поворота центр остаётся в (0;0).
* Для самопроверки включите контуру `core.getBBox()` в DevTools: координаты центра должны быть близки к нулю и не уходить за пределы рабочей области.

### Уникальные признаки (посадка гостей)

| Объект | Плановый признак | Комментарий |
|--------|------------------|-------------|
| Стол круглый Ø60/70/80 | Окружность + короткая «ножка» вниз | Линия длиной ~60 % радиуса подчёркивает стойку, сохраняя чистый контур. |
| Стол прямоугольный 120×70, 160×80 и т.д. | Прямоугольник со скруглениями + две короткие оси по центру коротких сторон | Оси смещены внутрь на 10–12 % габарита, что делает их видимыми, но не перегружает силуэт. |
| Хай-топ | Стандартный круглый стол + двойная дуговая метка со стороны бармена | Две концентрические кривые внутри окружности образуют «каплю», отличающую высокий стол от обычного. |
| Стул | Компактный прямоугольник + тонкая спинка одной линией | Спинка располагается вдоль верхнего ребра, а сиденье выделено лёгким внутренним прямоугольником. |
| Табурет | Малый круг/квадрат + центральная точка | Отсутствие спинки подчёркивается единственным маркером сиденья; контур остаётся цельным. |
| Банкетка прямая | Прямоугольник с широкой посадкой и линией-спинкой вдоль длинной стороны | Тонкая линия проходит внутри верхнего ребра, задавая ориентацию спинки. |
| Кабинка (будка) | Короб с открытым фасадом + стол внутри | U-образная линия повторяет контур посадки, центральный стол и вертикальная ось задают симметрию.
